...
//   let root_url = window.location.origin + "/"
  ...
  const get_dealer = async () => {
    try {
      ...
      const retobj = await res.json();
      if (retobj.status === 200) {
        setDealer(retobj.dealer); // ← directly, no Array.from()
      }
    } catch (err) {
  ...
  if (loading) return <div><Header /><p style={{margin:"20px"}}>Loading dealer details...</p></div>;
  if (!dealer)  return <div><Header /><p style={{margin:"20px"}}>Dealer not found.</p></div>;
